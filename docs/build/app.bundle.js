...
var _easyhttp = __webpack_require__(125);

var _games = __webpack_require__(330);
...
function changeState() {
	_ui.ui.showSubmitState();
}
//verifies that the input is a 17-digit number and not empty
function verifyInput(e) {
	var idInput = document.querySelector("#steamID").value;
	var re = /^[0-9]{17}?/;
	if (idInput === '' || !re.test(idInput)) {
		_ui.ui.showAlert('Please enter a valid steamID', 'error');
	} else {
...
function displayGames(steamid) {
	...
	_games.games.getPlayerName(steamid).then(function (response) {
		_ui.ui.displayName(response);
	}).catch(function (err) {
		_ui.ui.showAlert('Sorry, there was an error getting your information. Please check your steam community settings and make sure you have input the correct Steam64 ID.', 'error');
		console.log(err);
	});

	_games.games.getGames(steamid).then(function (response) {
		console.log(response);
		if (response === 'error') {
			_ui.ui.showError();
		} else {
			_ui.ui.paint(response);
		}
	}).catch(function (err) {
		_ui.ui.showAlert('Sorry, there was an error getting your information. Please check your steam community settings and make sure you have input the correct Steam64 ID.', 'error');
		console.log(err);
...
var Games = function () {
	...
	_createClass(Games, [{
		...
		value: function getGames(steamid) {
			...
			var sortedObj = void 0;
			var playerName = void 0;
			//run get request via http library, and return sorted object
			return _easyhttp.http.get('https://cors-anywhere.herokuapp.com/http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=' + this.APIkey + '&include_appinfo=1&include_played_free_games=1&steamid=' + this.steamID + '&format=json').then(function (results) {
				//check for error in results
				if (results.response.game_count) {
					//sort games via function
					sortedObj = _this.sortGames(results.response);
		...
		key: 'getPlayerName',
		value: function getPlayerName(steamid) {
			this.steamID = steamid;
			var playerName = void 0;
			//run get request via http library to get player's id
			return _easyhttp.http.get('https://cors-anywhere.herokuapp.com/http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=' + this.APIkey + '&steamids=' + this.steamID).then(function (results) {
				console.log(results);
		...
		key: 'sortGames',
		value: function sortGames(gamesObj) {
			if (gamesObj === 'error') {
				return gamesObj;
			} else {
				...
				var game_count = gamesObj.game_count;
				var gamesArray = gamesObj.games;
				var unplayedArr = [];
				var playedArr = [];
				var totalArr = gamesArray;
				...
				totalArr.sort(nameCompare);var data = {
					game_count: game_count,
					unplayedArr: unplayedArr,
					playedArr: playedArr,
					totalArr: totalArr
				};
...
var games = exports.games = new Games();
...
var UI = function () {
	...
	_createClass(UI, [{
		key: "showSubmitState",
		value: function showSubmitState() {
		...
		key: "paint",
		value: function paint(sortedObj) {
			var lpIndex = sortedObj.playedArr.length - 1;
		...
		value: function showAlert(message, className) {
			...
			//Get Parent
			var container = document.querySelector('.submit-state');
			//Get posts
		...
		value: function showError() {
			...
			var container = document.querySelector('.display-state');
